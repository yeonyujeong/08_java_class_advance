...
public class ATM {
	...
	void logout() {
		if(identifier == -1) {
			System.out.println("[메세지] 로그인을 하신 후 이용할 수 있습니다.");
		}
		else {
			identifier = -1;
	...
	void printAccountMenu() {
		...
		while(true) {
			...
			if(sel == 1) {
				...
				if(userManager.user[identifier].accCount == 0) {
					System.out.println("[메세지] '더이상 삭제할 수 없습니다.'\n" );
					continue;
				}
				
				if(userManager.user[identifier].accCount == 1) {
					System.out.println("[메세지] 계좌번호 : '" + userManager.user[identifier].acc[0].number + "'삭제되었습니다.\n");
					userManager.user[identifier].acc = null;
				}
				else {
					
					System.out.print("삭제하고싶은 계좌번호를 입력하세요 : ");
					...
					if(delIdx == -1) {
						...
						System.out.println("[메세지] 계좌번호 : '" + userManager.user[identifier].acc[delIdx].number + "'삭제되었습니다.\n");
